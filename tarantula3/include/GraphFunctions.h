...
cyclicedge getStartEdge(Graph *g,float ratioInteriorExterior) {
	...
	//edge_t ed_startEdge = getRandomEdge(g, graphstart, graphend, true);
		//FromEdgeListIntegrated(g, graphstart, graphend, ratioInteriorExterior);
...
cyclicedge getGoalEdge(Graph *g, vector<edge_t> connectableEdges , float ratioInteriorExterior) {
	cyclicedge goaledge;
	tie(ei, eiend) = boost::edges(*g);
	//edge_t ed_goalEdge = getRandomEdge(g, connectableEdges.begin(), connectableEdges.end(), true); // also accept forbidden edges
