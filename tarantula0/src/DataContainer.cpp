...
void DataContainer::addCyclesToVertices() {
...
edge_t DataContainer::connectAB(vertex_t endPointA, vertex_t endPointB, float rc, int ind, bool isforbidden) {
...
void DataContainer::emptyCyclesFromVertices() {
	for (std::tie(vi, viend) = boost::vertices(g); vi != viend; ++vi) {
		g[*vi].cycles.clear();
	}
}

void DataContainer::exportGraph(std::string directory) {
	std::ofstream myfile;
...
void DataContainer::exportGraphOBJ(Graph g, std::string directory) {
	std::ofstream myfile;
...
float DataContainer::getDivPoint(edge_t edge) {
...
std::optional<edge_t> DataContainer::GetEdgeFromItsVerts(vertex_t v, vertex_t u, Graph const& g)
{
...
edge_t DataContainer::getRandomEdgeFromEdgeListIntegrated(T& begin, T& end, bool forbcheck, float density, float length, float tension) {
	T cachebegin = begin;
	T cacheend = end;
...
float DataContainer::integrateEdge(edge_t edge, voxel_map voxelMap) {
...
void DataContainer::InitialWebFromObj(float rc, std::string filename) {
	bool hasCycle;
	std::ifstream MyReadFile(filename);
	std::string line;
	int numberOfVertices = 0;
	my_log.AddLog("[info] initializing starting graph.\n");
	if (MyReadFile.good()) {
		...
		my_log.AddLog("added %i vertices \n", numberOfVertices);
	}
	else {
		my_log.AddLog("[warning] Initial Graph not found. check in your directory.\n");
	}

}


void DataContainer::addRandomCyclicEdgeTesting(float rc) {
	...
	ed_startEdge = getRandomEdgeFromEdgeListIntegrated(graphstart, graphend, true, G_density, G_length, G_tension);
	...
	if (connedges.size()) {
		...
		ed_goalEdge = getRandomEdgeFromEdgeListIntegrated(connedges.begin(), connedges.end(), forbiddenCheck, G_density, G_length, G_tension); // also accept forbidden edges
...
std::vector<edge_t> DataContainer::getConnectedEdges(cyclicedge edge, std::vector<size_t>& edgeindices, bool forbCheck) {
	edge_t hs;
...
std::vector<edge_t> DataContainer::connectEdges(cyclicedge startedge, cyclicedge goaledge, float rc) {
...
void DataContainer::initEdge(edge_t ed, cyclicedge& cedge)
{
	cedge.descriptor = ed;
...
ci::vec3 DataContainer::interpolate(cyclicedge edge, float t) {
	return edge.start.pos + (edge.end.pos - edge.start.pos) * t;
}

void DataContainer::removebyindex(std::vector<size_t>& vec, size_t ind) {
	vec.erase(std::remove(vec.begin(), vec.end(), ind), vec.end());
}

void DataContainer::resolveIntersections(cyclicedge start, cyclicedge goal, size_t cycle1index, size_t cycle2index) {
...
std::vector<size_t> DataContainer::vectorcycles(std::vector<size_t> vec) {
...
void DataContainer::updatetext(std::string in) {
...
void DataContainer::shiftCycle(std::vector<size_t>& cycle, int index1, int  index2) {
...
void DataContainer::coordcont::calcvals() {
	start = start;
	end = end;
	direction = end - start;
	sign = sgn(direction);
...
bool DataContainer::coordcont::compareendvoxels() {
	return startvoxel != endvoxel;
}

bool DataContainer::coordcont::sortbystartvoxeldist(const coordcont& x, const coordcont& y) {
	return x.startvoxeldist < y.startvoxeldist;
}


void DataContainer::coordcont::addparam(int index, std::vector<std::array<float, 2>>& paramlist, ci::vec3& currentvoxel, voxel_map voxelMap) {
	if (compareendvoxels()) {
...
void DataContainer::addIntersectionVertexToCycles(cyclicedge& edge, int cycleIndex) {
...
void DataContainer::relaxPhysics() {
	float k = 1.1f;
	float eps = 0.1f;
...
edge_t DataContainer::pickrandomEdge(edge_ti& begin, edge_ti& end) {
...
void DataContainer::drawPoints(bool drawNumbers, bool drawVertexPoints, bool drawCycleList) {
...
void DataContainer::drawGraph(bool colorEdges, bool colorTension, bool colorLength) {
...
void DataContainer::setupCamera() {
	{
...
void DataContainer::drawNthCycle() {
	if (highlightCycle && !cycles.empty() && nthCycle < cycles.size()) {
		auto cycle = cycles[nthCycle];
...
void DataContainer::drawNthEdge() {
	if (highlightEdge) {
		ci::gl::color(ci::Color(1.0f, 0.0f, 0.0f));

		ci::gl::drawLine(g[boost::source(highlightedEdge, g)].pos, g[boost::target(highlightedEdge, g)].pos);
	}
}